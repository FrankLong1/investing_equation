...
A luminous chamber for each stage, surrounded by moving information.

Visual treatment: Particles / chambers. Inspired by the glowing geometry, particles, and full-screen composition of the local flashy-ai-visuals project. This is an exploratory presentation of the framework; decorative geometry does not represent live activity or an execution DAG.

## Run

From `visualizations/`, run `npm install` once and `npm run dev`. Open `/neon-studies/004-information-reactor/` on the local address shown in the terminal. The gallery switches among all installed experiments.

To work on this version directly: `npm run dev --workspace 004-information-reactor` from `visualizations/`. To build only this version: `npm run build --workspace 004-information-reactor`. The output is `neon-studies/004-information-reactor/dist/neon-studies/004-information-reactor/index.html`; run `npm run preview --workspace 004-information-reactor` and open `/neon-studies/004-information-reactor/` to check it. A standalone build contains this experiment; the gallery and other experiments are available through the development server.
