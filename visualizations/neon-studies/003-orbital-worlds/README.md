...
The framework held in one orbit. Enter a world to discover its sections.

Visual treatment: Orbits / constellations. Inspired by the glowing geometry, particles, and full-screen composition of the local flashy-ai-visuals project. This is an exploratory presentation of the framework; decorative geometry does not represent live activity or an execution DAG.

## Run

From `visualizations/`, run `npm install` once and `npm run dev`. Open `/neon-studies/003-orbital-worlds/` on the local address shown in the terminal. The gallery switches among all installed experiments.

To work on this version directly: `npm run dev --workspace 003-orbital-worlds` from `visualizations/`. To build only this version: `npm run build --workspace 003-orbital-worlds`. The output is `neon-studies/003-orbital-worlds/dist/neon-studies/003-orbital-worlds/index.html`; run `npm run preview --workspace 003-orbital-worlds` and open `/neon-studies/003-orbital-worlds/` to check it. A standalone build contains this experiment; the gallery and other experiments are available through the development server.
