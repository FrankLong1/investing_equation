...
A suspended field of functions. Drag, zoom, and follow the hierarchy.

Visual treatment: Lattice / depth. Inspired by the glowing geometry, particles, and full-screen composition of the local flashy-ai-visuals project. This is an exploratory presentation of the framework; decorative geometry does not represent live activity or an execution DAG.

## Run

From `visualizations/`, run `npm install` once and `npm run dev`. Open `/neon-studies/002-infinite-canvas/` on the local address shown in the terminal. The gallery switches among all installed experiments.

To work on this version directly: `npm run dev --workspace 002-infinite-canvas` from `visualizations/`. To build only this version: `npm run build --workspace 002-infinite-canvas`. The output is `neon-studies/002-infinite-canvas/dist/neon-studies/002-infinite-canvas/index.html`; run `npm run preview --workspace 002-infinite-canvas` and open `/neon-studies/002-infinite-canvas/` to check it. A standalone build contains this experiment; the gallery and other experiments are available through the development server.
